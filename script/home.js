const loadAllData = async (id=all) => {
    console.log(id)
    await fetch("https://phi-lab-server.vercel.app/api/v1/lab/issues")
     .then((allData) => allData.json())
    .then((viewData) => rcvData(viewData.data,id))
}
...
const searchD = document.getElementById('search-data');
...
const rcvData = (allData,idrcv) => {
    idrcv.innerHTML = '';
    allData.forEach(item => {
        if (idrcv === openn || idrcv === all){
            if (item.status === "open"){
                ...
                `
                idrcv.appendChild(div);
            }
        }
        
        if (idrcv === closed || idrcv === all){
...
const search = async (data) => {
    ...
    await fetch(`https://phi-lab-server.vercel.app/api/v1/lab/issues/search?q=${searchData}`)
    .then((searchItem) => searchItem.json())
    .then((searchValue) => searchLoad(searchValue.data));
}

const searchLoad = (rcvSearchData) => {

    all.classList.add('hidden');
    openn.classList.add('hidden')
    closed.classList.add('hidden')
    searchD.classList.remove('hidden')
    searchD.innerHTML = '';

    for (item of rcvSearchData) {
        if (item.status === "open"){
            const div = document.createElement('div');
            div.className = 'card-open';
            div.innerHTML = `
                <div onclick="my_modal_4.showModal();loadAlartInfo(${item.id});">
                    <div class="px-[8px] flex flex-col gap-2">
                        ...
                        <h2 class="title text-[14px]">${item.title}</h2>
                        <p class="text-[10px] opacity-[0.7]">${item.description}</p>
                        <div class="labels flex gap-2">
                            <p class="labels text-[9px] font-semibold px-3 py-[3px] rounded-xl">${item.labels[0]}</p>
                            <p class="labels text-[9px] font-semibold px-3 py-[3px] rounded-full">${item.labels[1]}</p>
    ...
    allCount.innerHTML = searchD.childElementCount;
    openCount.innerHTML = searchD.childElementCount;
    closedCount.innerHTML = searchD.childElementCount;
}
...
/* সকালে ঘুম থেকে uthe উপরের short logic ta mathy ashce logic mathay ashte pocur time lage
